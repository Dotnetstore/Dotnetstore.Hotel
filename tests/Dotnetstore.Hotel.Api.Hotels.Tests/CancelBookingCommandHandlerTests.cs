...
{
    private static BookingEntity NewReservedBooking()
    {
        ...
        return BookingEntity.Create(Guid.NewGuid(), customer, DateOnly.FromDateTime(DateTime.UtcNow), DateOnly.FromDateTime(DateTime.UtcNow.AddDays(1)), [room]);
    }

    [Fact]
    public async Task HandleAsync_ReservedBooking_CancelsAndSaves()
    {
        var booking = NewReservedBooking();
        ...
        var handler = new CancelBookingCommandHandler(repository.Object, unitOfWork.Object);
        var result = await handler.HandleAsync(new CancelBookingCommand(booking.Id), CancellationToken.None);

        result.Succeeded.ShouldBeTrue();
        booking.Status.ShouldBe(BookingStatuses.Cancelled);
        unitOfWork.Verify(u => u.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task HandleAsync_AlreadyCheckedOutBooking_ReturnsErrorAndDoesNotSave()
    {
        var booking = NewReservedBooking();
        booking.CheckIn();
        booking.CheckOut();
    ...
    public async Task HandleAsync_BookingDoesNotExist_ReturnsErrorAndDoesNotSave()
    {
        ...
        var handler = new CancelBookingCommandHandler(repository.Object, unitOfWork.Object);
        var result = await handler.HandleAsync(new CancelBookingCommand(Guid.NewGuid()), CancellationToken.None);
