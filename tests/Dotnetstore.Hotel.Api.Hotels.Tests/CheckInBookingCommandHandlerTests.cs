...
public class CheckInBookingCommandHandlerTests
{
    ...
    [Fact]
    public async Task HandleAsync_ReservedBooking_ChecksInAndSaves()
    {
        ...
        var handler = new CheckInBookingCommandHandler(repository.Object, unitOfWork.Object);
        var result = await handler.HandleAsync(new CheckInBookingCommand(booking.Id), CancellationToken.None);

        result.Succeeded.ShouldBeTrue();
        booking.Status.ShouldBe(BookingStatuses.CheckedIn);
        unitOfWork.Verify(u => u.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task HandleAsync_CancelledBooking_ReturnsErrorAndDoesNotSave()
    {
        var booking = NewReservedBooking();
        booking.Cancel();
    ...
    public async Task HandleAsync_BookingDoesNotExist_ReturnsErrorAndDoesNotSave()
    {
        ...
        var handler = new CheckInBookingCommandHandler(repository.Object, unitOfWork.Object);
        var result = await handler.HandleAsync(new CheckInBookingCommand(Guid.NewGuid()), CancellationToken.None);
